...
###### Source file for Boechera_Ecotypes_Microbiome analyses
                          ...
                          ###### Ecotypes Source Script ###### 

## Color palettes
sitePalette <- c("#000000", "#E69F00", "#56B4E9")
popPalette <- c("#CC79A7", "goldenrod", "#0072B2","#009E73","grey","#F0E442","lavender",
                "aquamarine3","darkorchid","cadetblue3","#D55E00")
agePalette <- c("lightskyblue","blue","black","red")

## Load packages (or install it first if you don't have them)
library(tidyr)
library(maps)
library(mapdata)
library(ape)
...
library(scales)
library(lme4)
# library(DESeq2) #Faltó
library(lmerTest)
library(reshape2)
# library(BiocParallel) #Faltó
library(VennDiagram)

## Register parallel backend. Configure the number of cores you will use for running parallel loops
...
## Function: r2.LMM() => estimate R^2 for a model as correlation between fitted and 
## observed values
r2.LMM <- function(m) {
...
LMMstats<-function(data,resp,reml=TRUE){
  df<-data
  stats<-data.frame('Response'=character(0),'Term'=character(0), 'F or ChiSq'= numeric(0),
                    'df' = integer(0), 'ddf'=numeric(0),'P'=numeric(0))
  model<-lmer(df[[resp]]~Genotype*Site + Age*Site + Harvested*Site
              +(1|Site:Block)+(1|Genotype:Line)+(1|newPlate)+logObs,data=df,REML=reml)
