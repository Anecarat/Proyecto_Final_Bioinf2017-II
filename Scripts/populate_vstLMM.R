...
# R script to create separate working directories for each OTU and put an OTU-specific data frame 
# (containing all M columns of metadata + 1 column of count data) into each working directory. 
...
# Load the data
df.leaf<-readRDS("df_leaf_vst.rds")
df.root<-readRDS("df_root_vst.rds")

# How many columns of metadata are there? ADJUST AS NEEDED
colnames(df.leaf)
...
for (i in 1:dim(leafKeys)[1]){
  ...
  system(paste("cp countmodels_vstLMM.R ./leaf/",i,"/countmodels_vstLMM.R",sep="")) # Copy statistics script into new directory for current taxon
...
for (i in 1:dim(rootKeys)[1]){
  ...
  system(paste("cp countmodels_vstLMM.R ./root/",i,"/countmodels_vstLMM.R",sep="")) # Copy statistics script into new directory for current taxon
  rm(thistaxon,thisdata) # clean up
}

####### Write bash script to start job array for leaf taxa ######
sink('start_countmodels_vstLMM_leaf.q')
cat("#!/bin/bash\n#\n")
...
system("sbatch start_countmodels_vstLMM_leaf.q")
system("sbatch start_countmodels_vstLMM_root.q")
